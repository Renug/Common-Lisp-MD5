...
(defun left-rotate (num rotate-bit &optional (length-in-bit 32))
  (if (and (numberp num) (numberp rotate-bit))
      (let ((right-part (ldb (byte rotate-bit (- length-in-bit rotate-bit)) num))
	    (left-part (ldb (byte (- length-in-bit rotate-bit) 0) num)))
    ...
    (loop for lst in 512bit-list 
	 do(let ((dword-list (mapcar #'reverse  (combo-list lst))))
		  (dotimes (i 63)
		    (cond 
		      ((and (<= 0 i) (<= i 15))
		       (setf f (logior (logand b c) (logand (lognot b) d)))
		       (setf g i))
		      ((and (<= 16 i) (<= i 31))
		       (setf f (logior (logand d b) (logand (lognot d) c)))
		       (setf g (mod (1+ (* 5 i)) 16)))
		      ((and (<= 32 i) (<= i 47))
		       (setf f (logxor b c d))
		       (setf g (mod (+ 5 (* 3 i)) 16)))
		      ((and (<= 48 i) (<= i 63))
		       (setf f (logxor c (logior b (lognot d))))
		       (setf g (mod (* 7 i) 16))))
		    (let ((temp (32integer d)))
		      (setf d (32integer c))
		      (setf c (32integer b))
		      (incf b (32integer (left-rotate 
