...
(defvar *r-list* '(7 12 17 22 7 12 17 22 7 12 17 22  7 12 17 22 5  9 14 20  5  9 14 20  5  9 14 20  5  9 14 20 4 11 16 23  4 11 16 23  4 11 16 23  4 11 16 23 6 10 15 21  6 10 15 21  6 10 15 21  6 10 15 21))
  ...
  (let ((ret-list (copy-list lst)))
    ...
    (let ((len (* (length lst) 1)))
	  (loop for x from 0 to 7 do 
	       (nappend ret-list (list (ldb (byte 8 (* x 8)) len)))))
    ret-list))

(defun md5 (string)
  (labels ((combo-list (lst &optional (sublist-length 4))
	     (if (or (null lst) (< (length lst) sublist-length))
		 lst
		 (append (list (subseq lst 0 sublist-length)) (combo-list (subseq lst  sublist-length)))))
	   (byte-to-dword (lst)
	     (let ((result 0))
  ...
  (let ((512bit-list (combo-list (pre-processing (string-to-code-list string)) 64))
    ...
    (loop for lst in 512bit-list 
	 do(let ((dword-list (mapcar #'reverse (apply #'combo-list 512bit-list))))
	     (format t "~x" (byte-to-dword '(#xab #xcd #x12 #x34)))
		  (dotimes (i 63)
		    (cond 
		      ((and (<= 0 i) (<= i 15))
		    ...
		    (let ((temp d))
		      (setf d c)
		      (setf c b)
		      (incf b (left-rotate (+ a f (elt *k-list* i) (byte-to-dword (elt dword-list g))) (elt *r-list* i)))
		      (setf a temp)))
		  (incf h0 a)
		  (incf h1 b)
		  (incf h2 c)
